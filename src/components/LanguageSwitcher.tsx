...
export function LanguageSwitcher() {
  ...
  const switchTo = (target: (typeof locales)[number]) => {
    setOpen(false);
    // Full navigation: the locale is resolved in the root layout (html lang,
    // LocaleProvider and the whole site chrome), which is reused from the
    // router cache during a client-side navigation — leaving the UI in the
    // previous locale. A hard navigation forces a fresh server render so the
    // selected locale applies immediately on every page.
    window.location.assign(localizePath(pathname, target));
  };
